...
export default {
	...
	props: {
		...
		// 权限校验 ---有效值查看https://ext.dcloud.net.cn/plugin?id=594
		permission: {
			type: Object,
			default:()=>{return {
		...
		// 用户未授权时是否使用组件内置弹框引导用户去系统开启权限,若传false，可通过@permissionFail回调自行处理
		// 跳转系统界面表可查看 https://ask.dcloud.net.cn/question/14732
		isPermissionBuiltInHandle: {
			type: Boolean,
			default: true
		},
		// 微信选择文件类型 
	...
	mounted() {
		...
		this.lsjFile = new LsjFile({
			...
			prohibited: {
				...
				isPermissionBuiltInHandle: this.isPermissionBuiltInHandle
			},
