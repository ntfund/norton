<template>
	<view>
		<com-nav-bar title="活期申购" :right="tools.hideAccount(userStore.user.account, 2, 4)" leftWidth="220rpx" rightWidth="220rpx" ></com-nav-bar>
		<view class="group_4 flex flex flex-col">
			...
			<view class="section_3 flex flex-row justify-between">
				...
				<view class="balance flex items-center" v-if="data.payType == 1">
					<view class="balance_name">钱包余额</view>
					<view class="balance_value flex">:&nbsp;
						...
						<!-- <view v-else style="color: #7F17FE;" @click="showWallet = true">请切换至BNB钱包</view> -->
					</view>
				</view>
			...
			<view class="section_5 flex flex-col justify-start">
				...
				<view class="info-item flex flex-row items-center justify-between">
					...
					<view class="text_17">
						<span class="font-[SourceHanSansCN-Regular]"><!-- 每天 -->{{t('deposit.float.p7')}}</span>
					</view>
				</view>
				<view class="info-item flex flex-row items-center justify-between">
				...
				<view class="info-item flex flex-row items-center justify-between">
					...
					<span class="text_17"><!--随存随取-->{{t('deposit.float.p10')}}</span>
				</view>
				<view class="info-item flex flex-row items-center justify-between">
				...
				<view class="info-item flex flex-row items-center justify-between">
					<span class="text_16"><!--日收益率-->{{t('deposit.float.p12')}}</span>
					<span class="text_17">{{data.rate}}%</span>
				</view>

			</view>
			<view class="mt-[131rpx] m-button">
				<view class="text-center text-[24rpx] text-gray-500 tip" v-if="tip">{{tip}}</view>
				<button type="button" class="n-btn" :class="{'disabled': loading || disabled}" :disabled="loading || disabled" @click="submit">
					确认并申购
				</button>
			</view>
		</view>
...
<script setup>
	...
	const disabled = computed(() => {
		...
		return !!(!data.buyAmount || parseFloat(data.buyAmount) < parseFloat(data.min) || !b1);
	})
