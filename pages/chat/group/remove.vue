<template>
	<uni-nav-bar :fixed="true" :border="false" left-icon="left" @clickLeft="clickLeft" backgroundColor="#F8F8F8"
		:statusBar="true" title="选择群成员">
		<template #right>
			<view>
				<u-button type="primary" :disabled="loading" color="red" text="删除" size="small"
...
<script setup lang="ts">
	...
	const rightClick = async () => {
		...
		loading.value = true;
		uni.$u.http.post('/chat/chatRoom/remove/' + props.id, { ids }).then(data => {
			uni.showToast({
				title: '移除群成员成功'
			})
			loading.value = false;
	...
	uni.$u.http.get('/chat/chatRoom/users/' + props.id).then(data => {
