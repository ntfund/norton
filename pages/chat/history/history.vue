<template>
	...
	<view class="msg-list">
		<view class="row">
			<view class="other " v-for="(row, index) in msgList" :key="index">
				<!-- 左-头像 -->
				<view class="left" @click="tools.navigateTo('/pages/chat/user/info?id=' + row.msg.userinfo.id)">
					<image :src="getAvatarPath(row.msg.userinfo.avatar)" mode="aspectFit" style="background-color: #fff; width: 50px; height: 50px; border-radius: 10rpx;">
					</image>
				...
				<view class="right">
					...
					<view v-if="row.msg.type=='img'" class="bubble img" @tap="showPic(row.msg)">
						<image :src="http.config.baseURL + row.msg.content.url" mode="widthFix" style="max-width: 200px;">
						</image>
					</view>
					<!-- 名片消息 -->
...
<script setup>
	...
	const getData = () => {
		uni.$u.http.get('/chat/message/' + props.id).then(res => {
			console.log(res)
			uni.setNavigationBarTitle({
				title: res.data.msg.content.title
			})
			msgList.value = [];
