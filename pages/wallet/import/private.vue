	...
	<scroll-view class="content_scroll-view" scroll-y="true" upper-threshold="50">
		<view class="content">
			<view class="flex content_tip">
				<view class="pr-[5rpx]">输入明文私钥或扫码二维码，请注意大小写。为了您的资产安全</view>
				<view class="content_logo">Norton</view>
				<view class="pl-[5rpx]">不会存储您的私钥，请妥善保管！</view>
			</view>
			<view class="input-box">
				<textarea class="input-box_textarea" v-model="formData.privateKey" placeholder="输入明文私钥或扫码二维码，请注意大小写"
					cursor-spacing="20" />
			...
			<com-wallet-verify v-model:wallet="formData"></com-wallet-verify>
...
<script setup>
	...
	const disabled = computed(() => {
		if (hasPassword.value) {
			return !formData.privateKey || !formData.name;
		}
		return !formData.privateKey || !formData.name || formData.password != formData.password2 || formData
			.password.length < 8;
	})
