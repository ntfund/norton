<template>
	<view>
		...
		<com-wallet-tab v-model:current="current" :list="['Keystore', '二维码']" style="margin: 0 28rpx;"></com-wallet-tab>
		<view class="content">
			<view class="text-1">获取Keystore和密码等于拥有钱包资产所有权</view>
			<view v-if="current == 0">
				<view class="input-box">
					<textarea class="input-box_textarea" :value="keystore" placeholder="Keystore" disabled
						cursor-spacing="20" />
				</view>
			</view>
			<view v-else class="pt-[38rpx]">
				<com-qrcode :data="keystore"></com-qrcode>
			</view>
		...
		<view class="btn-bottom">
			<button type="button" class="n-btn " @click="ok">拷贝 Keystore</button>
		</view>
	</view>
</template>
...
<script setup>
	...
	const keystore = secret.decrypt(wallet.keystore)
	const keystore_password = secret.decrypt(wallet.keystore_password)


	const ok = () => {
		tools.copyClick(keystore)
	}
</script>
