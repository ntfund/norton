<template>
	<uni-nav-bar :fixed="true" :border="false" left-icon="left" left-width="30px" backgroundColor="#F8F8F8"
		...
		<template #default>
			<view style="display: flex; flex-direction: row; justify-items: start; align-items: center;">
				...
				<view class="ml-3">
					<view class="font-black text-base">购买 USDT</view>
	...
	<view class="p-4 mt-4 text-sm">
		...
		<view class="mt-4 p-4 bg-white rounded-xl ">
			<view class="font-black">支付方式</view>
	...
	<view class="boottom-container">
		<view class="px-4">
			<button type="button" class="rounded-full" @click="buy">0手续费买入USDT</button>
		</view>
	</view>
...
<script setup lang="ts">
	...
	const tabList = ref([{ name: '按金额购买' }, { name: '按数量购买' }]);
	...
	const buy = () => {
		...
		uni.$u.http.post('/c2cOrder', formData).then(data => {
			tools.navigateToWithEncode('/pages/market/buy/buying', data)
		})
	}
