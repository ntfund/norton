<template>
	...
	<com-nav-bar title="" backgroundColor="#ffffff" @clickLeft="tools.navigateTo('/pages/market/shop/order')"></com-nav-bar>
	<view class="px-4 text-sm pb-10">
		...
		<view class="text-xl font-black mt-4">已转账，等待卖家确认转账</view>
	...
	<view class="btns">
		<view class="btn cancel">等待卖家确认转账</view>
	</view>
	...
	const getData = () => {
		uni.$u.http.get('/c2cOrder/' + props.id).then(data => {
			...
			if (data.status == C2COrderType.Success) {
				...
				clearInterval(timer)
				uni.reLaunch({
					url: '/pages/market/shop/sell/success?id=' + props.id
				})
			}
