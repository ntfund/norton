<template>
	<view>
		<com-input-max label="出售数量" v-model="formData.amount" :max="usdtToken.balance"
			placeholder="请输入数量"></com-input-max>

		<com-input type="digit" label="出售价格" v-model="formData.price" right="CNY"
			placeholder="请输入价格"></com-input>

		<view class="flex justify-between items-center">
			<com-input type="digit" label="单笔限额" v-model="formData.min_cny" right="CNY"
				placeholder="最小数"></com-input>
			<view class="gang"></view>
			<com-input class="self-end" type="digit" v-model="formData.max_cny" right="CNY"
				placeholder="最大数"></com-input>
		</view>	
		

		<view class="mb-[30rpx]">收款方式</view>
		<view class="">
		...
		<u-modal :show="show" title="删除委托" content='确认删除吗？删除后将解冻该委托金额.' confirmText="删除" showCancelButton
			confirmColor="#ff0000" @confirm="deleteConfirm" @cancel="show = false" closeOnClickOverlay
	...
	export default {
		...
		methods: {
			...
			add() {
				...
				// if (this.formData.id == 0 && parseFloat(this.formData.max_cny) > parseFloat(this.usdtToken.balance)) {
				// 	return uni.$u.toast("最大金额超过可用金额");
				// }
				if (!this.formData.price) {
					return uni.$u.toast("请输入价格");
				}
				if (parseFloat(this.formData.max_cny) > parseFloat(this.cny)) {
					return uni.$u.toast("您的单笔CNY上限为：" + this.cny);
				}
				if (this.formData.payments.length == 0) {
					return uni.$u.toast("请最少选择一种收款方式");
				}
			...
			// 		return uni.$u.toast("请输入最小数量");
			// 	}
			// 	if (this.formData.min_amount < 1) {
			// 		return uni.$u.toast("最小数量不得小于1");
			// 	}
			// 	if (parseFloat(this.formData.max_amount) < parseFloat(this.formData.min_amount) || parseFloat(this.formData.max_amount) <= 1) {
			// 		return uni.$u.toast("区间范围不正确");
			// 	}
			// 	if (!this.formData.price) {
			// 		return uni.$u.toast("请输入价格");
			...
			// 		return uni.$u.toast("请最少选择一种收款方式");
			// 	}
			// 	uni.$u.http.put('/c2c/' + this.formData.id, this.formData).then(res => {
			// 		uni.navigateBack();
			// 		uni.$u.toast("更新成功");
			// 	})
			// },
			deleteConfirm() {
				uni.$u.http.delete('/c2c/' + this.formData.id).then(balances => {
