window.Buffer = (function() {
	const base64 = (() => {
		...
		function toByteArray(b64) {
			...
			for (i = 0; i < len; i += 4) {
				tmp = (revLookup[b64.charCodeAt(i)] << 18) | (revLookup[b64.charCodeAt(i +
					1)] << 12) | (revLookup[b64.charCodeAt(i + 2)] << 6) | revLookup[b64
					.charCodeAt(i + 3)]
		...
		function tripletToBase64(num) {
			return lookup[num >> 18 & 0x3F] + lookup[num >> 12 & 0x3F] + lookup[num >> 6 &
				0x3F] + lookup[num & 0x3F];
		}
		...
		function encodeChunk(uint8, start, end) {
			...
			for (var i = start; i < end; i += 3) {
				tmp = ((uint8[i] << 16) & 0xFF0000) + ((uint8[i + 1] << 8) & 0xFF00) + (uint8[
					i + 2] & 0xFF)
		...
		function fromByteArray(uint8) {
			...
			if (extraBytes === 1) {
				...
				parts.push(lookup[tmp >> 10] + lookup[(tmp >> 4) & 0x3F] + lookup[(tmp << 2) &
					0x3F] + '=');
			}
	...
	function utf8Slice(buf, start, end) {
		...
		while (i < end) {
			...
			let codePoint = null
			let bytesPerSequence = (firstByte > 0xEF) ? 4 : ((firstByte > 0xDF) ? 3 : ((firstByte >
				0xBF) ? 2 : 1))
					...
					case 3:
						...
						if ((secondByte & 0xC0) === 0x80 && (thirdByte & 0xC0) === 0x80) {
							tempCodePoint = (firstByte & 0xF) << 0xC | (secondByte & 0x3F) << 0x6 | (
								thirdByte & 0x3F)
							if (tempCodePoint > 0x7FF && (tempCodePoint < 0xD800 || tempCodePoint >
									0xDFFF)) {
								codePoint = tempCodePoint
							}
						}
	...
	Buffer.prototype.writeBigInt64LE = defineBigIntMethod(function writeBigInt64LE(value, offset = 0) {
		return wrtBigUInt64LE(this, value, offset, -BigInt('0x8000000000000000'), BigInt(
			'0x7fffffffffffffff'))
	})

	Buffer.prototype.writeBigInt64BE = defineBigIntMethod(function writeBigInt64BE(value, offset = 0) {
		return wrtBigUInt64BE(this, value, offset, -BigInt('0x8000000000000000'), BigInt(
			'0x7fffffffffffffff'))
	})
	...
	function boundsError(value, length, type) {
		...
		throw new errors.ERR_OUT_OF_RANGE(type || 'offset', `>= ${type ? 1 : 0} and <= ${length}`,
			value)
	}
